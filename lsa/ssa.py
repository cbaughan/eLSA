...
def submit( slurmFile ):
  tmp=subprocess.Popen("sbatch %s" % (slurmFile), shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
  submitted=True
  if tmp[1] == '':
    print tmp[0]
    print slurmFile, "submitted"
  else:
    print tmp[1]
    print slurmFile, "error"
    submitted=False
  return submitted
...
def main():
  ...
  slurmFiles = set([slurmFile])

  #print slurmFiles

  while( len(slurmFiles) != 0 ):
    if peek_current(uname):  #full
      time.sleep(1)
    else:
      slurmFile=slurmFiles.pop()
      print len(slurmFiles)
      if not submit(slurmFile):
        slurmFiles.add(slurmFile)
